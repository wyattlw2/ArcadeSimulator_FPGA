...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int returncode = alt_avalon_spi_command(SPI0_BASE, 0, 2, wdata, 0, NULL, 0);
	//if return code < 0 print an error
	if (returncode<0){
		printf("MAXreg_WRerror");
	}
	return;
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int returncode = alt_avalon_spi_command(SPI0_BASE, 0,nbytes+1, wdata,0, NULL,0);

	//if return code < 0 print an error
	if (returncode<0){
		printf("MAXbytes_WRerror");
	};
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE *val = NULL;
		//write val via SPI
	const BYTE* regpntr=&reg;
		//read return code from SPI peripheral (see Intel documentation)
	int returncode=alt_avalon_spi_command(SPI0_BASE, 0,1,regpntr,1, val,0);
	//if return code < 0 print an error
	if (returncode<0){
		printf("MAXreg_RDerror");
	}
	return *val;
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	const BYTE* regpntr=&reg;
			//read return code from SPI peripheral (see Intel documentation)
		int returncode=alt_avalon_spi_command(SPI0_BASE, 0,1,regpntr,nbytes,data,0);
	//if return code < 0 print an error
		if (returncode<0){
			printf("MAXbytes_RDerror");
		};
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	return (data + nbytes);
}
